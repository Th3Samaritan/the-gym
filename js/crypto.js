/* ============================================================
   crypto.js — password-derived keys, AES-GCM encrypt/decrypt,
   admin RSA-OAEP recovery.

   Uses ONLY the browser-native Web Crypto API.
   No external libraries, no npm dependencies.
   ============================================================ */
...
export function configureAdminKey(b64) {
  ADMIN_PUBLIC_KEY_B64 = b64;
}
...
/**
 * Derive a 32-byte AES-256 key from a password + username.
 * Same password + username always produces the same key.
 */
export async function deriveKey(password, username) {
  const salt = encoder.encode(SALT_PREFIX + username.toLowerCase().trim());
...
/** Fast hash for login verification — stored in the encrypted blob. */
export async function passwordHash(password) {
  const buf = await crypto.subtle.digest('SHA-256', encoder.encode(password));
  return bufToB64(new Uint8Array(buf));
}

/* ------------------------------------------------------- AES-GCM encrypt */

/**
 * Encrypt profile data with the user's derived key.
 * Returns { iv, encrypted, passwordHash } where iv and encrypted are base64.
 */
export async function encryptProfile(userKey, profile, password) {
  const iv = crypto.getRandomValues(new Uint8Array(12));
  const key = await crypto.subtle.importKey('raw', userKey, 'AES-GCM', false, ['encrypt']);
  const plaintext = encoder.encode(JSON.stringify(profile));
  const ciphertext = await crypto.subtle.encrypt({ name: 'AES-GCM', iv }, key, plaintext);
  const hash = await passwordHash(password);
  return {
    iv: bufToB64(iv),
    encrypted: bufToB64(new Uint8Array(ciphertext)),
    passwordHash: hash,
  };
}

/**
 * Decrypt profile data with the user's derived key.
 * Returns the parsed profile object, or null on failure.
 */
export async function decryptProfile(userKey, encryptedB64, ivB64) {
  try {
    const key = await crypto.subtle.importKey('raw', userKey, 'AES-GCM', false, ['decrypt']);
    const ciphertext = b64ToBuf(encryptedB64);
...
/**
 * Encrypt the user's AES key with the admin's RSA public key.
 * This lets the admin decrypt the user's data without knowing their password.
 */
export async function adminEncryptUserKey(userKey) {
  if (!ADMIN_KEY_CONFIGURED()) return null;
  try {
    const pubKey = await crypto.subtle.importKey(
      'spki', b64ToBuf(ADMIN_PUBLIC_KEY_B64),
      { name: 'RSA-OAEP', hash: 'SHA-256' }, false, ['encrypt']
    );
    const ciphertext = await crypto.subtle.encrypt({ name: 'RSA-OAEP' }, pubKey, userKey);
    return bufToB64(new Uint8Array(ciphertext));
  } catch {
    return null;
  }
}
...
/**
 * Decrypt a user key with the admin's RSA private key.
 * Used in the GitHub Action or admin recovery tool.
 */
export async function adminDecryptUserKey(privateKeyB64, adminRecoveryB64) {
  const privKey = await crypto.subtle.importKey(
    'pkcs8', b64ToBuf(privateKeyB64),
    { name: 'RSA-OAEP', hash: 'SHA-256' }, false, ['decrypt']
  );
  const ciphertext = b64ToBuf(adminRecoveryB64);
